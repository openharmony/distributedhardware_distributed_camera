...
int32_t Capture()
{
    ...
    if (ret != DCAMERA_OK) {
        DHLOGE("main g_photoOutput Capture failed, ret: %d", ret);
        return ret;
    }
    return DCAMERA_OK;
}
...
int32_t Video()
{
    ...
    if (ret != DCAMERA_OK) {
        DHLOGE("main VideoOutput Start failed, ret: %d", ret);
        return ret;
    }
    return DCAMERA_OK;
}
...
int32_t GetPreviewProfiles(std::vector<CameraStandard::Size> &previewResolution)
{
    ...
    DHLOGI("size: %d", previewProfiles.size());
    for (auto& profile : previewProfiles) {
        CameraStandard::Size picSize = profile.GetSize();
        DHLOGI("width: %d, height: %d", picSize.width, picSize.height);
        if (IsValid(picSize)) {
