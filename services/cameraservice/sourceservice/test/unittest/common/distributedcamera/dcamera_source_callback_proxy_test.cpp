...
HWTEST_F(DcameraSourceCallbackProxyTest, dcamera_source_callback_proxy_test_004, TestSize.Level1)
{
    ...
    EXPECT_EQ(DCAMERA_OK, callbackProxy->OnNotifyRegResult(maxdirstr, testemptystr, testemptystr, status, emptystr));
    EXPECT_EQ(DCAMERA_OK, callbackProxy->OnNotifyRegResult(teststr, testemptystr, testemptystr, status, emptystr));
    EXPECT_EQ(DCAMERA_OK, callbackProxy->OnNotifyRegResult(teststr, maxdirstr, testemptystr, status, emptystr));
    EXPECT_EQ(DCAMERA_OK, callbackProxy->OnNotifyRegResult(teststr, teststr, testemptystr, status, emptystr));
    EXPECT_EQ(DCAMERA_OK, callbackProxy->OnNotifyRegResult(teststr, teststr, maxdirstr, status, emptystr));
    EXPECT_EQ(DCAMERA_OK, callbackProxy->OnNotifyRegResult(teststr, teststr, teststr, status, maxdirstr1));
}
