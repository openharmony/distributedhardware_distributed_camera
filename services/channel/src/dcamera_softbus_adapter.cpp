...
int32_t DCameraSoftbusAdapter::CreateSoftbusSessionServer(std::string sessionName, DCAMERA_CHANNEL_ROLE role)
{
    ...
    if (ret != DCAMERA_OK) {
        ...
        ReportSoftbusSessionServerFail(PKG_NAME, sessionName, "create session server failed");
        return ret;
    }
    sessionTotal_[sessionName]++;
    ...
    int32_t sessionMode, std::string peerDevId)
{
    ...
    if (sessionId < 0) {
        ...
        ReportSoftbusSessionFail(mySessName, peerSessName, peerDevId, "open softbus session failed.");
